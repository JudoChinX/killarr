# Contributing to Killarr

Thank you for considering contributing to Killarr! This document provides guidelines for contributing to the project and helps ensure a smooth collaboration process.
...
- Use GitHub Security Advisories: https://github.com/JudoChinX/killarr/security.
- Or email: killarr@judochinx.com for non-GitHub users.

See [SECURITY.md](SECURITY.md) for our full security policy and coordinated disclosure timeline.

## Development Setup

For detailed setup instructions, see the [User Guide](docs/user-guide.md#development-setup).
...
Killarr handles API keys and communicates with self-hosted services. Security is paramount:
...
If your contribution adds or changes API endpoint calls, update [SECURITY.md](SECURITY.md) and [docs/technical-audit.md](docs/technical-audit.md) accordingly.
...
| Type checking | `mypy killarr/ tests/` |
| Code quality | `pylint killarr/ tests/` |
| Security scan | `bandit -r killarr/ -lll` |
| YAML lint | `yamllint .` |
| Dependency audit | `pip-audit -r requirements.txt` |
| Test suite | `pytest` (95% coverage required) |

These are the same checks you can run locally. If CI fails on your PR, run the failing check locally to reproduce it — the output is identical.

Security-related failures (Bandit) block merge regardless of other results.

Thank you for contributing to Killarr!
