...
# ── prompt for version ─────────────────────────────────────────────────────────
read -rp "Release version (e.g. 0.1.0): " VERSION

if [[ ! "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  abort "Version must match MAJOR.MINOR.PATCH (e.g. 0.1.0). Got: $VERSION"
fi
...
git commit -m "new: Killarr v${VERSION} release."
git push -u origin "$RELEASE_BRANCH" || { runbook_hint; exit 1; }

echo ""
echo "Release branch pushed. Next steps:"
echo "  1. Open a PR: gh pr create --base main --head $RELEASE_BRANCH --title 'new: Killarr v${VERSION} release.'"
