...
Step-by-step process for tagging and publishing a new Killarr release.
...
2. Builds and pushes a multi-arch Docker image (`linux/amd64`, `linux/arm64`) to Docker Hub as both `judochinx/killarr:<version>` and `judochinx/killarr:latest`
...
| Docker push | Image pushed to `judochinx/killarr:X.Y.Z` and `judochinx/killarr:latest` |
...
2. **Docker Hub** — confirm the new tag and `latest` are present at `hub.docker.com/r/judochinx/killarr`
